...
function StaffCard2({ employee }) {
    ...
    return (
        <div className="mt-8 relative flex flex-col items-start gap-4 bg-white rounded-xl z-0 max-w-[400px]">
            ...
            >
                <img src={employee.image} alt="Staff"
                    className="w-full h-full object-cover transform group-hover:scale-110 group-hover:-rotate-3 group-focus:scale-110 group-focus:-rotate-3 group-active:scale-110 group-active:-rotate-3 transition-all duration-500 ease-in-out"
                />
                ...
                <div
                    className="absolute bottom-6 right-10 flex flex-col gap-2 transform translate-x-24
                    group-hover:translate-x-0 group-focus:translate-x-0 group-active:translate-x-0 
                    transition-all duration-700 ease-in-out"
                >
                    {[FaFacebook, FaXTwitter, FaInstagram, FaYoutube].map((Icon, i) => (
                        <div
                            key={i}
                            className="bg-white text-primary hover:bg-primary hover:text-white focus:bg-primary focus:text-white active:bg-primary active:text-white rounded-md p-3 cursor-pointer transition-all duration-300"
                        >
